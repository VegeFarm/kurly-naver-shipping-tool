...
async def naver_confirm(job_id: str, body: ConfirmBody):
    ...
    try:
        ...
        # 브라우저에서 단순 Internal Server Error 대신 실제 네이버 인증/API 원인을 확인할 수 있게 한다.
        raise HTTPException(status_code=502, detail=f"네이버 API 연결 실패: {exc}") from exc
    ...
    try:
        ...
        raise HTTPException(
            status_code=500,
            detail=f"네이버 발주확인은 처리됐지만 로컬 완료기록 저장에 실패했습니다: {exc}",
        ) from exc
    return {
